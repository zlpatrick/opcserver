...
{
    class SerialToOPCServer
    {
        private static SerialToOPCServer server;
        Dictionary<string, SerialPort> PortMapping = new Dictionary<string, SerialPort>();

        private SerialToOPCServer()
        {
        }

        public static SerialToOPCServer getInstance()
        {
            if (server == null)
                server = new SerialToOPCServer();
            return server;
        }

        public void startSerialMonitor()
        {
            LoadPort();
